...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      callback(
        new Error(
          "CORS Policy Block: Origin not authorized."
        )
      );
    },
...
const server =
  app.listen(
    ...
    () => {
      console.log(
        `[SYSTEM] Magnifinder API Engine running on 0.0.0.0:${PORT}`
      );
    }
  );
  ...
  server.close(
    async () => {
      try {
        ...
        console.log(
          "[SYSTEM] Prisma Client disconnected. Process terminated successfully."
        );

        process.exit(0);
      } catch (error: unknown) {
        console.error(
          "[SYSTEM] Failed to disconnect Prisma cleanly:",
          error
        );
