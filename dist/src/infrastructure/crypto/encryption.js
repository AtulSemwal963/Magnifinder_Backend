...
const KEY_LENGTH = 32;
const KEY_ENVIRONMENT_VARIABLE = "CREDENTIAL_ENCRYPTION_KEY";
...
const getEncryptionKey = () => {
    const rawKey = process.env[KEY_ENVIRONMENT_VARIABLE]?.trim();
    if (!rawKey) {
        throw new Error(`${KEY_ENVIRONMENT_VARIABLE} is not configured.`);
    }
    if (!/^[0-9a-fA-F]+$/.test(rawKey) ||
        rawKey.length !== KEY_LENGTH * 2) {
        throw new Error(`${KEY_ENVIRONMENT_VARIABLE} must be a 32-byte hexadecimal key.`);
    }
