...
import { getOpenOutFindConfig } from "./openoutfind.config.js";
/**
...
class OpenOutFindClient {
    config;
    constructor(config = getOpenOutFindConfig()) {
        this.config = config;
    }
    /**
     ...
     */
    getCampaignDatabasePath(campaignId) {
     ...
     */
    executeOpenOutFind(databasePath, input) {
        return new Promise((resolve, reject) => {
            ...
            const child = spawn(this.config.executable, ["--db", databasePath, "find", String(input.count)], {
                env: environment,
            ...
            child.on("close", (exitCode, signal) => {
                ...
                if (exitCode === 0) {
                    resolve({ stdout, stderr, exitCode, signal: signal });
                    return;
                }
     ...
     */
    detectConfigurationError(stdout, stderr) {
        const combinedOutput = `${stdout}\n${stderr}`;
        const normalizedOutput = combinedOutput.toLowerCase();
        const hasConfigurationError = OPENOUTFIND_CONFIGURATION_ERRORS.some((pattern) => normalizedOutput.includes(pattern.toLowerCase()));
     ...
     */
    extractMissingEnvironmentVariables(output) {
     ...
     */
    extractFailureDetails(stderr, stdout) {
        ...
        if (errorLine)
            return errorLine.replace(/^error\s*:\s*/i, "").trim();
    ...
    isCsvHeader(line) {
        ...
        return (normalized.startsWith("email,") &&
    ...
    validateInput(input) {
        if (!input || typeof input !== "object")
            throw new Error("OpenOutFind input is required.");
        if (typeof input.campaignId !== "string" || !input.campaignId.trim())
            throw new Error("Campaign ID is required.");
        if (typeof input.productDocs !== "string" || !input.productDocs.trim())
            throw new Error("Product documentation is required.");
        if (typeof input.campaignTarget !== "string" || !input.campaignTarget.trim())
            throw new Error("Campaign target is required.");
        if (!Number.isInteger(input.count) || input.count <= 0)
            throw new Error("Lead count must be a positive integer.");
        if (input.count > 1000)
            throw new Error("Lead count cannot exceed 1000.");
        if (!input.credentials || typeof input.credentials !== "object")
            throw new Error("OpenOutFind credentials are required.");
        if (typeof input.credentials.aiModel !== "string" || !input.credentials.aiModel.trim())
            throw new Error("OpenOutFind AI model is required.");
        if (typeof input.credentials.llmApiKey !== "string" || !input.credentials.llmApiKey.trim())
            throw new Error("OpenOutFind LLM API key is required.");
        if (typeof input.credentials.betterContactApiKey !== "string" || !input.credentials.betterContactApiKey.trim())
            throw new Error("BetterContact API key is required.");
        if (typeof input.credentials.operatorEmail !== "string" || !input.credentials.operatorEmail.trim())
            throw new Error("OpenOutFind operator email is required.");
        if (!input.runtime || typeof input.runtime !== "object")
            throw new Error("OpenOutFind runtime configuration is required.");
        if (typeof input.runtime.country !== "string" || !input.runtime.country.trim())
            throw new Error("OpenOutFind country is required.");
        if (typeof input.runtime.acceptLegalNotice !== "boolean")
            throw new Error("OpenOutFind legal notice acceptance is required.");
        if (!input.runtime.acceptLegalNotice)
            throw new Error("OpenOutFind legal notice must be accepted.");
    }
    /**
     ...
     */
    deriveNameFromLinkedInUrl(linkedinUrl) {
        ...
        const match = trimmedUrl.match(/linkedin\.com\/in\/([^/?#]+)/i);
        if (!match?.[1]) {
            return {
                firstName: "",
                lastName: "",
        ...
        const formatNamePart = (part) => part.length > 0
    ...
    createHeaderIndexes(headers) {
        ...
        for (let index = 0; index < headers.length; index++) {
            ...
            if (CSV_HEADERS.includes(normalizedHeader)) {
...
//# sourceMappingURL=openoutfind.client.js.map