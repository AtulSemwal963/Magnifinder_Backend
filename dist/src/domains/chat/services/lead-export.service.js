...
import { StreamingXlsxWriter, } from "../../../infrastructure/export/xlsx/index.js";
...
export class LeadExportService {
    xlsxWriter;
    constructor(xlsxWriter = new StreamingXlsxWriter()) {
        this.xlsxWriter =
            xlsxWriter;
    }
    /**
    * Export all leads belonging to every search in a chat as an XLSX
     ...
     * in this service.
     */
    async exportChatLeads(chatId, stream) {
        this.validateChatId(chatId);
        this.validateStream(stream);
        const searches = await searchRepository.findManyByChatId(chatId);
        const searchIds = searches.map((search) => search.id);
        const data = this.createLeadExportStream(searchIds);
...
//# sourceMappingURL=lead-export.service.js.map