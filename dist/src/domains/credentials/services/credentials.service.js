import { encrypt, decrypt, } from "../../../infrastructure/crypto/index.js";
...
export class CredentialsService {
    ...
    async createCredential(userId, input) {
        ...
        const credential = await credentialsRepository.create({
            userId,
            provider,
            ...(modelName !== undefined && {
                modelName,
    ...
    async getCredential(userId, provider) {
        ...
        const credential = await credentialsRepository.findByUserIdAndProvider(userId, normalizedProvider);
        if (!credential) {
            return null;
        }
        return this.toPublicCredential(credential);
    ...
    async updateCredential(userId, provider, input) {
        ...
        if (input.apiKey !== undefined) {
            ...
            if (trimmedApiKey) {
                const encryptedApiKey = encrypt(trimmedApiKey);
        ...
        if (Object.keys(updateData).length === 0) {
            throw new Error("At least one credential field must be provided.");
        }
        const credential = await credentialsRepository.updateByUserIdAndProvider(userId, currentProvider, updateData);
        return this.toPublicCredential(credential);
    }
    /**
    ...
    async deleteCredential(userId, provider) {
        this.validateUserId(userId);
        const normalizedProvider = this.validateProvider(provider);
        await credentialsRepository.deleteByUserIdAndProvider(userId, normalizedProvider);
    }
    // ==========================================================
     ...
     */
    async getDecryptedCredential(userId, provider) {
        this.validateUserId(userId);
        const normalizedProvider = this.validateProvider(provider);
        const credential = await credentialsRepository.findByUserIdAndProvider(userId, normalizedProvider);
        if (!credential) {
    ...
    validateOptionalModelName(modelName) {
        if (modelName === undefined) {
            return undefined;
        }
