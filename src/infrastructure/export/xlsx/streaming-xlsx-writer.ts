...
/**
 * Plain lead representation accepted by the XLSX exporter.
 *
 * This intentionally does not import Prisma, OpenOutFind,
 * chat, repository, or domain types.
 */
...
/**
 * A single JSON-compatible export record.
 *
 * The exporter can therefore remain reusable for future
 * export types while the lead-specific contract above
 * provides type safety for this particular export.
 */
export type XlsxRecord =
  LeadExportRecord;

/**
 * Configuration for a single exported column.
 */
export interface XlsxColumn {
  /**
   * Object property containing the value.
   */
  key: keyof LeadExportRecord;

  /**
   * Human-readable Excel column heading.
   */
  header: string;

  /**
   * Optional Excel column width.
   */
  width?: number;

  /**
   * Render this value as an Excel hyperlink.
   */
  hyperlink?: boolean;
}
 ...
 */
const LEAD_COLUMNS: readonly XlsxColumn[] = [
  {
    key: "firstName",
    header: "First Name",
...
/**
 * Input required to generate a lead XLSX workbook.
 *
 * The data is an AsyncIterable so the caller can eventually
 * stream leads directly from a database cursor without
 * materializing the complete dataset in memory.
 */
export interface WriteXlsxInput {
  /**
   * Destination stream.
   *
   * Usually the HTTP response stream.
   */
  stream: Writable;
   ...
   * Lead data source.
   */
  data: AsyncIterable<LeadExportRecord>;
}
...
export class StreamingXlsxWriter {
   ...
   */
  private createRowValues(
    lead: LeadExportRecord
  ): Record<
    string,
  ...
  private validateInput(
    ...
    if (
      ...
      throw new Error(
        "XLSX data must be an AsyncIterable."
      );
    }
  }
}
