...
/**
 * Identity established by the server-side Google OAuth
 * verification layer.
 *
 * This type is intentionally separate from the request body.
 *
 * The browser must never be allowed to construct this object.
 */
...
/**
 * Data required when completing application onboarding.
 *
 * Username is the only user-controlled identity field required
 * at this stage.
 */
...
/**
 * Public application identity returned to the rest of the
 * application.
 *
 * OAuth credentials/identifiers are deliberately not exposed.
 */
...
class IdentityService {
  ...
  async createAccount(
    ...
    const oauthId =
      input.googleIdentity.oauthId.trim();

    const username =
      input.username.trim();

    const email =
      input.googleIdentity.email?.trim() ||
      null;
      ...
      const user =
        await prisma.user.create({
          data: {
            ...
            email,
            googleImage:
              input.googleIdentity.googleImage ??
              null,
          },
          select: {
            id: true,
