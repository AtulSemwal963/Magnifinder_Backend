...
import {
  leadExportService,
} from "../services/lead-export.service.js";
...
export class ChatController {
  ...
  async generateLeads(
    ...
    try {
      ...
      const input =
        req.body as GenerateLeadsRequest;

      const result =
        await chatService.generateLeads(
          userId,
          chatId,
          input
        );
...
async exportLeads(
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${this.createExportFileName(
        chat.name
      )}"`
    );
     ...
     */
    await leadExportService.exportChatLeads(
      chatId,
      res
    );
  } catch (error: unknown) {
